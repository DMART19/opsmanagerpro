...
-- Add action_context column to error_logs for storing the action + sanitized inputs
ALTER TABLE public.error_logs
  ADD COLUMN IF NOT EXISTS action_context jsonb DEFAULT NULL;

-- Update the upsert function to accept action_context
CREATE OR REPLACE FUNCTION public.upsert_error_log(
  ...
  p_action_context jsonb DEFAULT NULL::jsonb
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF existing_id IS NOT NULL THEN
    ...
    SET hit_count = hit_count + 1,
        ...
        action_context = COALESCE(p_action_context, action_context)
