CREATE OR REPLACE FUNCTION bump_workspace_cache_version()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_old jsonb;
BEGIN
  -- Use JSONB to safely access fields without hard column references
  ...
  -- Determine user_id from the row (safe field access via jsonb)
  ...
  IF TG_TABLE_NAME = 'cache_inventory' THEN
    IF COALESCE((v_new->>'asset_type'), (v_old->>'asset_type')) = 'item' THEN
